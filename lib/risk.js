...
function calculatePositionSize(portfolioComponent, signal, currentPrice, winStreakCount = 0) {
  ...
  // Check 1: Max open positions limit (strict: max 3 for concentrated bets)
  ...
  // Check 2: Signal conviction scaling
  let baseRiskFactor = CONFIG.DEFAULT_RISK_PER_TRADE_PCT; // 15%
  if (signal.strength === 5 || signal.strength === -5) {
    baseRiskFactor = 0.25; // 25% allocation on STRONG_BUY
  } else if (signal.strength === 3 || signal.strength === -3) {
    baseRiskFactor = 0.15; // 15% allocation on BUY
  } else {
  ...
  // ══════════════════════════════════════════════════════════════════════
  // Scale allocation up on winning streaks (Anti-Martingale compounding)
  // Reset back to baseline on any loss (protects capital during drawdowns)
  ...
  const finalRiskFactor = Math.min(0.30, baseRiskFactor * streakMultiplier);

  // Desired compounding allocation in USD (Dynamic compounding on totalValuation)
