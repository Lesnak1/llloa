...
// Risk Parameters (Optimized Competition Mode — Capital Preservation + Growth)
const CONFIG = {
  MAX_SINGLE_ASSET_EXPOSURE_PCT: 0.15, // Max 15% of total portfolio in 1 asset (prevents concentration risk)
  MAX_OPEN_POSITIONS: 8,               // Max 8 concurrent positions for diversification
  MIN_ORDER_USD: 100,                  // Min order size to avoid dust orders
  DEFAULT_RISK_PER_TRADE_PCT: 0.05,    // Risk 5% of total equity per standard trade
...
function calculatePositionSize(portfolioComponent, signal, currentPrice) {
  ...
  if (signal.strength === 5 || signal.strength === -5) {
    riskFactor = 0.08; // 8% allocation on STRONG high-conviction signals
  } else if (signal.strength === 3 || signal.strength === -3) {
    riskFactor = 0.05; // 5% allocation on standard BUY/SELL
  } else {
