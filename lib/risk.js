...
function calculatePositionSize(portfolioComponent, signal, currentPrice, winStreakCount = 0) {
  ...
  // Check 2: Signal conviction scaling (Strict Capital Preservation)
  let baseRiskFactor = CONFIG.DEFAULT_RISK_PER_TRADE_PCT; // 10%
  if (signal.strength === 5 || signal.strength === -5) {
    baseRiskFactor = 0.15; // 15% allocation on STRONG_BUY
  } else if (signal.strength === 3 || signal.strength === -3) {
    baseRiskFactor = 0.10; // 10% allocation on BUY
  } else {
