...
// Risk Parameters
const CONFIG = {
  MAX_SINGLE_ASSET_EXPOSURE_PCT: 0.25, // Max 25% of total portfolio in 1 asset
  MAX_OPEN_POSITIONS: 5,               // Max 5 concurrent positions
  MIN_ORDER_USD: 100,                  // Min order size to avoid dust orders
  DEFAULT_RISK_PER_TRADE_PCT: 0.04,    // Risk 4% of total equity per high-conviction trade
  MAX_SPREAD_PCT: 0.03,                // Max allowable spread (3%) before skipping trade
};
...
function calculatePositionSize(portfolioComponent, signal, currentPrice) {
  ...
  if (signal.strength === 5 || signal.strength === -5) {
    riskFactor = 0.06; // 6% risk on STRONG signals
  } else if (signal.strength === 3 || signal.strength === -3) {
    riskFactor = 0.03; // 3% risk on standard BUY/SELL
  } else {
