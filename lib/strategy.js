...
function shouldClosePosition(position, currentPrice, analysis) {
  const entry = Number(position.avgEntryPrice || position.averagePrice || 0);
  ...
  // Hard stop-loss: -5% (allows asset room to breathe through minor noise)
  if (pnlPct <= -5) {
    return { shouldClose: true, reason: `Stop-loss hit: ${pnlPct.toFixed(2)}% loss` };
  }

  // Take profit: +10% (maximizing upside for competition ranking)
  if (pnlPct >= 10) {
    return { shouldClose: true, reason: `Take-profit hit: ${pnlPct.toFixed(2)}% gain` };
  }

  // Trailing stop: if we were up >3% and signal reverses or drops back
  if (pnlPct >= 3) {
  ...
  // Deep emergency loss exit: -7%
  if (pnlPct <= -7) {
    return { shouldClose: true, reason: `Emergency loss threshold hit: ${pnlPct.toFixed(2)}%` };
  }
