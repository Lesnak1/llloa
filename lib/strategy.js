...
function analyzeAsset(candles, tokenName) {
  ...
  // ── Map composite score to signal (High-Conviction Filtered Signals) ──
  let signal, strength;
...
function shouldClosePosition(position, currentPrice, analysis) {
  ...
  if (!entry || !qty) return { shouldClose: false, reason: 'No valid position data', minSellPrice: 0 };
  ...
  // Maximum allowed loss per trade is -8%. This prevents catastrophic fills
  // like the metlife $194.82 -> $155.00 disaster (-20.44%).
  // Bot.js must use Math.max(bestBid, minSellPrice) when placing limit sells.
  const MAX_LOSS_FLOOR_PCT = 0.08; // 8% max loss floor
  const minSellPrice = isLong ? Math.round(entry * (1 - MAX_LOSS_FLOOR_PCT) * 100) / 100 : 0;

  // ── Hard Stop-Loss: -4.0% ──
  // Prevents 1.5% orderbook bid/ask spread noise from triggering premature stops
  if (pnlPct <= -4.0) {
    return { shouldClose: true, reason: `Stop-loss hit: ${pnlPct.toFixed(2)}% loss`, minSellPrice };
  }

  // ── Take Profit: +7.0% ──
  // Captures multi-candle property token swings
  if (pnlPct >= 7.0) {
    return { shouldClose: true, reason: `Take-profit hit: ${pnlPct.toFixed(2)}% gain`, minSellPrice };
  }

  // ── Trailing Stop with Profit Lock ──
  // At +3.5% or higher: lock in at least half the gain on ANY signal weakness
  if (pnlPct >= 3.5) {
    // Exit on any bearish or even neutral (HOLD) signal — lock profits
    if (isLong && (analysis.signal === Signal.SELL || analysis.signal === Signal.STRONG_SELL || analysis.signal === Signal.HOLD)) {
      return { shouldClose: true, reason: `Trailing profit-lock at ${pnlPct.toFixed(2)}% gain (signal: ${analysis.signal})`, minSellPrice };
    }
  ...
  // At +2.0%: exit only on strong contrary signal
  if (pnlPct >= 2.0) {
    if (isLong && (analysis.signal === Signal.SELL || analysis.signal === Signal.STRONG_SELL)) {
      return { shouldClose: true, reason: `Trailing stop + reversal at ${pnlPct.toFixed(2)}% gain`, minSellPrice };
    }
  ...
  // ── Signal-Based Exit: strong contrary signal at any PnL ──
  if (isLong && analysis.signal === Signal.STRONG_SELL) {
    return { shouldClose: true, reason: `Strong sell signal on long position`, minSellPrice };
  }
  if (!isLong && analysis.signal === Signal.STRONG_BUY) {
    return { shouldClose: true, reason: `Strong buy signal on short position`, minSellPrice };
  }

  return { shouldClose: false, reason: `Holding: ${pnlPct.toFixed(2)}% PnL`, minSellPrice };
}
