...
function shouldClosePosition(position, currentPrice, analysis) {
  ...
  // Trailing stop: if we were up >2% and now dropped back
  if (pnlPct >= 2 && pnlPct < 3) {
    // Check if signal reversed
    if (isLong && (analysis.signal === Signal.SELL || analysis.signal === Signal.STRONG_SELL)) {
      return { shouldClose: true, reason: `Trailing stop + bearish reversal at ${pnlPct.toFixed(2)}%` };
    }
    if (!isLong && (analysis.signal === Signal.BUY || analysis.signal === Signal.STRONG_BUY)) {
      return { shouldClose: true, reason: `Trailing stop + bullish reversal at ${pnlPct.toFixed(2)}%` };
    }
  }
  ...
  // Deep loss exit: -5%
  if (pnlPct <= -5) {
    return { shouldClose: true, reason: `Max loss threshold: ${pnlPct.toFixed(2)}%` };
  }
