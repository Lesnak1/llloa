...
function shouldClosePosition(position, currentPrice, analysis) {
  ...
  // Optimized Tournament Parameters (+7.0% TP, -4.0% SL)
  // Hard stop-loss: -4.0% (prevents 1.5% orderbook bid/ask spread noise from triggering premature stops)
  if (pnlPct <= -4.0) {
    return { shouldClose: true, reason: `Stop-loss hit: ${pnlPct.toFixed(2)}% loss` };
  }

  // Take profit: +7.0% (captures multi-candle property token swings)
  if (pnlPct >= 7.0) {
    return { shouldClose: true, reason: `Take-profit hit: ${pnlPct.toFixed(2)}% gain` };
  }

  // Trailing stop: if we are up >2.5% and signal reverses or drops back
  if (pnlPct >= 2.5) {
