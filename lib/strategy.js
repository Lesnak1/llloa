...
function analyzeAsset(candles, tokenName) {
  ...
  // ══════════════════════════════════════════════════════════════
  // ══ AGGRESSIVE BREAKOUT SIGNAL MAPPING (v3 Thresholds)      ══
  // ══════════════════════════════════════════════════════════════
  let signal, strength;
...
function shouldClosePosition(position, currentPrice, analysis) {
  ...
  // ── Hard Stop-Loss: -4.0% (gives room for orderbook spread, 6.25:1 RR) ──
  if (pnlPct <= -4.0) {
    return { shouldClose: true, reason: `Stop-loss hit: ${pnlPct.toFixed(2)}% loss` };
  }

  // ── Take Profit: +25.0% (captures ultra-volatility mega-swings!) ──
  if (pnlPct >= 25.0) {
    return { shouldClose: true, reason: `Take-profit hit: ${pnlPct.toFixed(2)}% mega-gain!` };
  }

  // ── Trailing Profit-Lock at +8.0% (Rides mega-rallies up to +40%+) ──
  // If we hit +8% gain, lock profits if signal weakens or reverses
  if (pnlPct >= 8.0) {
    if (isLong && (analysis.signal === Signal.SELL || analysis.signal === Signal.STRONG_SELL || analysis.signal === Signal.HOLD)) {
      return { shouldClose: true, reason: `Mega-rally profit-lock at ${pnlPct.toFixed(2)}% (signal: ${analysis.signal})` };
    }
